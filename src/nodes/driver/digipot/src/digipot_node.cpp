...
void digipot_callback(const digipot::DigipotDataMsg& command) {
    ...
    if (fake_digipot) {
        ...
        if (result.substr(0, 1) != "U") {
            ROS_ERROR_STREAM(
                    "Digipot: expected 'U' reply, got " << result << " length "
                            << result.length());
        }
...
// catch an invalid port error
    ...
    if (!fake_digipot) {
        ...
        std::string result = my_serial->read(5);
        ROS_INFO_STREAM(
                "Digipot: startup reply: " << result << " length "
                        << result.length());
        if (result.substr(0, 1) != "U") {
            ROS_ERROR_STREAM(
                    "Digipot: expected 'U' reply to default setting, got "
                            << result);
        }
    } else {
        ROS_WARN_STREAM(
                "Digipot: continuing to run with fake connectivity for testing.");
    }
