    ...
    while (ros_interface.isNodeRunning()) {
        ...
        for (int y = 0; y < image.rows; y++) //Set up number of rows to loop through
            ...
            for (int x = 0; x < image.cols; x++) //Set up number of columbs to loop through
                    {
                if ((image.at < cv::Vec3b > (y, x)[RED] - RED_THRESHOLD)
                        > image.at < cv::Vec3b > (y, x)[GREEN]
                        && (image.at < cv::Vec3b > (y, x)[RED] - RED_THRESHOLD)
                                > image.at < cv::Vec3b > (y, x)[BLUE])
                ...
                if ((image.at < cv::Vec3b > (y, x)[GREEN] - GREEN_THRESHOLD)
                        > image.at < cv::Vec3b > (y, x)[BLUE]
                        && (image.at < cv::Vec3b
                                > (y, x)[GREEN] - GREEN_THRESHOLD) > image.at
                                < cv::Vec3b > (y, x)[RED])
        ...
        findContours(ired, r_contours, hierarchy, RETR_TREE,
        ...
        for (size_t i = 0; i < r_contours.size(); i++) {
            ...
            if (r_boundRect[i].width > WIDTH_MIN
                    && (abs(
                            1.0
                                    - (1.0 * r_boundRect[i].width
                                            / r_boundRect[i].height))
                            < SQUARE_TEST)) {
                // Draw the areas that match the size and shape threshold
                rectangle(image, r_boundRect[i].tl(), r_boundRect[i].br(),
                        color, 3, 8, 0);
            }
        }
        
        // rectangles around yellow
        findContours(iyellow, y_contours, hierarchy, RETR_TREE,
                CHAIN_APPROX_SIMPLE, Point(0, 0));
        ...
        for (size_t i = 0; i < y_contours.size(); i++) {
            ...
            if (y_boundRect[i].width > WIDTH_MIN
                    && (abs(
                            1.0
                                    - (1.0 * y_boundRect[i].width
                                            / y_boundRect[i].height))
                            < SQUARE_TEST)) {
                // Draw the areas that match the size and shape threshold
                rectangle(image, y_boundRect[i].tl(), y_boundRect[i].br(),
                        color, 3, 8, 0);
            }
        }
        ...
        for (size_t ir = 0; ir < r_contours.size(); ir++) {
            if (r_boundRect[ir].width > WIDTH_MIN
                    && (abs(
                            1.0
                                    - (1.0 * r_boundRect[ir].width
                                            / r_boundRect[ir].height))
                            < SQUARE_TEST)) {
                // look for matching yellow but only on right-sized red
                for (size_t iy = 0; iy < y_contours.size(); iy++) {
                    if ((abs(
                            ((1.0 * r_boundRect[ir].tl().x
                                    - y_boundRect[iy].tl().x)
                                    / r_boundRect[ir].width))
                            < HORIZONTAL_TOLERANCE) // yellow on top of red
                            && (r_boundRect[ir].br().y > y_boundRect[iy].tl().y) // red on top of yellow
                            && (abs(
                                    1.0
                                            - (1.0 * r_boundRect[ir].width
                                                    / y_boundRect[iy].width))
                                    < WIDTH_TOLERANCE) // similar size
                            && (r_boundRect[ir].width > WIDTH_MIN)
                            && (y_boundRect[iy].width > WIDTH_MIN) // minimum size
                            && (abs(
                                    1.0
                                            - (1.0 * y_boundRect[iy].width
                                                    / y_boundRect[iy].height))
                                    < SQUARE_TEST) // also square
                            && ((r_boundRect[ir].tl().x - y_boundRect[iy].tl().x)
                                    << 2 * r_boundRect[ir].width) // not too far apart
                            ) {
                        int pixels_offset;
                        Scalar color = Scalar(0, 200, 200);
                        rectangle(image, y_boundRect[iy].tl(),
                                y_boundRect[iy].br(), color, 5, 8, 0);
                        line(image, y_boundRect[iy].tl(), y_boundRect[iy].br(),
                                color, 5, 8, 0);
                        rectangle(image, r_boundRect[ir].tl(),
                                r_boundRect[ir].br(), color, 5, 8, 0);
                        line(image, r_boundRect[ir].tl(), r_boundRect[ir].br(),
                                color, 5, 8, 0);
                        target_found = true;
                        pixels_offset = (r_boundRect[ir].br().x
                                - (image.cols / 2) + y_boundRect[iy].width / 2);
                        target_direction = atan(
                                pixels_offset / BASE_PIXEL_EQUIVALENT);
                        target_distance = TARGET_SPACING * BASE_PIXEL_EQUIVALENT
                                / (r_boundRect[ir].tl().y
                                        - y_boundRect[iy].tl().y) - 1;
                    }
                }
            }
        }
        ...
        if (target_found) {
            ros_interface.publishMessages(target_found, target_direction,
                    target_distance);
            no_target_count = 0;
        } else {
