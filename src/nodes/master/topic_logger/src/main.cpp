...
//     std::string data = "Compass heading: "
//             + std::to_string(compassMessage.heading);
    
//     writeData(compass_file, data);
// }

// void gps_callback(const bruin2_msgs::GPSDataMsg& gpsMessage) {
//     std::string data = "Compass latitude: "
//             + std::to_string(gpsMessage.latitude) + '\n' + "	longitude: "
//             + std::to_string(gpsMessage.longitude) + '\n' + "	altitude: "
//             + std::to_string(gpsMessage.altitude); // + '\n'
...
//     std::string data = "Camera direction: "
//             + std::to_string(cameraMessage.direction) + '\n' + "	distance: "
//             + std::to_string(cameraMessage.distance) + '\n' + "	valid: "
//             + std::to_string(cameraMessage.tracking);
...
int main(int argc, char **argv) {
    ...
    // ROSInterface ros_interface(&relay_callback, &compass_callback,
    //         &gps_callback, &camera_callback);
    ...
    // relay_file = std::ofstream(base_path + "relay_board_msgs.log",
    //         std::ios_base::app);
    // compass_file = std::ofstream(base_path + "compass_msgs.log",
    //         std::ios_base::app);
    // gps_file = std::ofstream(base_path + "gps_msgs.log", std::ios_base::app);
    // camera_file = std::ofstream(base_path + "camera_msgs.log",
    //         std::ios_base::app);
