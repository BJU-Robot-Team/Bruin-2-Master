...
inline bool queryUserForTransition(StateMachine* state_machine, 
  std::vector<std::string> line_desc, std::vector<VehicleEvents> event_lines){

    //declare we are in debug mode
    std::cout << "Debug Mode Active (query user for transition): ";
    ...
    do {
        std::cout << "Debug Mode Active: Please select one of the options below:";
        ...
        int answer_int = std::stoi (answer_str,&size);

        //no number was given or answer is not one of the options 
        if (size == 0 || answer_int > i || answer_int < 0) {
            continue;
        }
