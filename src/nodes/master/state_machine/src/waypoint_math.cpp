...
double WaypointMath::calculateThetaW(double x1, double y1, double x2, double y2) {
  double theta_w = atan((y2-y1)/(x2-x1));
  

  if (x2 > x1 && y2 > y1) { return theta_w; } // quadrant I
  else if (x2 < x1 && y2 > y1) { return (theta_w + pi_num); } // quadrant II
  else if (x2 < x1 && y2 < y1) { return (theta_w + pi_num); } // quadrant III
  else                         { return (theta_w + (2*pi_num)); } // quadrant IV (x2 > x1 && y2 < y1)
...
double WaypointMath::calcSteerAngle(double &theta_c, double &theta_w, double k) {
	...
	if (theta_d > 0) {
		...
		else { negVar = -1,std::cout << "Turn Left" << std::endl; }
	}
	else {
		if (std::abs(theta_d) < pi_num) { negVar = -1, std::cout << "Turn Left" << std::endl; }
