...
void WaitForCommandState::tick(StateMachine* state_machine,
    ...
    if (vehicle_data->selected_station != "None"
...
bool WaitForCommandState::debugState(StateMachine* state_machine) {
    ...
    return queryUserForTransition("WAIT_FOR_COMMAND", state_machine, line_desc,
            event_lines);
}
