...
bool WaypointMap::parseCSVData(std::string file_path,
        bool file_type_waypoints) {
    ...
    if (file.open(file_path, std::ios::in)) {
        ...
        if (file_type_waypoints == false) {
            ...
            //  station index
            while (waypoint_reader.read_row(comment, latitude, longitude,
                    curvature, precision, stop_sign_value)) {
            ...
            findStationIndexes(start_station, end_station, start_station_index,
                    end_station_index);
            ...
            if (start_station_index == -1 || end_station_index == -1) {
                ...
                station_matrix[start_station_index][end_station_index] =
                        wp_path;
            }
        }
        ...
        throw std::runtime_error(
                "Waypoint map loading is missing a file, path: " + file_path);
    }
    return false;
}

//returns a waypoint path given two stations
WaypointPath *WaypointMap::getWaypointList(std::string start_station,
...
void WaypointMap::findStationIndexes(std::string start_station,
        std::string end_station, int& start_station_index,
        int& end_station_index) {
    //go through the station index list and find what index the start
    //  and end stations have
    int i = 0;
    for (auto it = station_index.begin(); it != station_index.end();
            it++, i++) {
