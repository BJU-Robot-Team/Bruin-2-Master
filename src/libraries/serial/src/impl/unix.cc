...
Serial::SerialImpl::SerialImpl(const string &port, unsigned long baudrate,
        ...
        port_(port), fd_(-1), is_open_(false), xonxoff_(false), rtscts_(false), baudrate_(
                baudrate), parity_(parity), bytesize_(bytesize), stopbits_(
                stopbits), flowcontrol_(flowcontrol) {
...
size_t Serial::SerialImpl::read(uint8_t *buf, size_t size) {
    ...
    total_timeout_ms += timeout_.read_timeout_multiplier
    ...
    while (bytes_read < size) {
        ...
        if (waitReadable(timeout)) {
            ...
            ssize_t bytes_read_now = ::read(fd_, buf + bytes_read,
                    size - bytes_read);
...
size_t Serial::SerialImpl::write(const uint8_t *data, size_t length) {
    ...
    total_timeout_ms += timeout_.write_timeout_multiplier
